...
def checkAnswer(title, userAnswer, Answer):
    ...
    if (userAnswer == Answer):
        print ("  Congratulations!",pGreen(str(Answer)),"is indeed the correct answer!")
    else:
        print ("  Sorry, but",pRed(str(userAnswer)), "is wrong,",pGreen(str(Answer)),"is the correct answer.")

def main():
    print("INTRODUCTION")
    print("This program will show the steps involved in creating a shared secret using the Diffie Hellman algorithm between two parties.")
    print("In this example the parties are Alice and Bob, a well-known couple in Cryptography")
    ...
    print("  calculate base key",pBlue("(g)"),"as a primitive root modulo of",pBlue("(p)"))
    print("  calculate her public key",pBlue("(A)"),"using the following formula:",pYellow("(g)^(a) MOD (p)"))
    print("  share",pBlue("(g), (p)"),"and her public key",pBlue("(A)"),"with",pGreen("Bob"))
    print(pGreen("Bob"),"will")
    print("  generate his private key",pBlue("(b)"))
    print("  calculate his public key",pBlue("(B)"),"using the following formula:",pYellow("(g)^(b) MOD (p)"))
    ...
    print("   to calculate their shared secret key",pBlue("(k)"),"using the following formula:",pYellow("(a or b)^(A or B) MOD (p)"))
    isDone()
    print("It should be obvious that the longer the keys, the harder the challenge to either calculate (or break) Diffie Hellman.")
    print("This program will challenge you to a competition in finding the shared secret",pBlue("(k)"),"of",pGreen("Alice and Bob"))
    ...
    print("")
    print (pGreen("Alice"))
    print ("  generates public base key",pBlue("(g)"),"and public prime key",pBlue("(p)"),"...")
    pubBaseKey = -1
    while (pubBaseKey < 0):
    ...
    print(pGreen("Alice"))
    print("  shares with",pGreen("Bob"),"public base key",pBlue("(g)"),":",pBlue(str(pubBaseKey)),"and public prime number",pBlue("(p)"),":",pBlue(str(pubPrimeKey)),"...")
    isDone()

    print(pGreen("Alice and Bob"))
    print("  use their private keys",pBlue("(a, b)"),";and")
    print("  the public prime number",pBlue("(p)"),":",pBlue(str(pubPrimeKey)))
    print("  the public base number",pBlue("(g)"),":",pBlue(str(pubBaseKey)))
    ...
    print("  exchange their public keys",pBlue("(A, B)"),":",pBlue(str(pubA)),"and",pBlue(str(pubB)))
    print("  Alice uses Bob's public key",pBlue("(B)"),":",pBlue(str(pubB)),"to calculate their shared secret",pBlue("(k)"),"...")
    thr_kA = ThreadWithReturnValue(target=calcKey, args=(privA, pubB, pubPrimeKey,))
    thr_kA.start()
    print ("  Bob uses Alice's public key",pBlue("(A)"),":",pBlue(str(pubA)),"to calculate their shared secret",pBlue("(k)"),"...")
    ...
    print ("  Checks if Alice and Bob have indeed the same shared secret",pBlue("(k)"),"...")
    if (kA!=kB):
        print ("   ",pRed("ERROR"),": shared secret",pBlue("(k)"),"is not the same")
        exit(1)
    else:
        print ("   ",pGreen("SUCCES"),"Alice and Bob have indeed calculated the same secret",pBlue("(k)"))
    isDone()

    print("Now comes the time to test your human modulo skills...")
    print("Let's compete with regular CPU", pMagenta("Mallory"),"to find either")
    ...
    print("  tries to calculate Alice's and Bob's private key",pBlue("(a, b)"),"using public known",pBlue("(A), (B), (p) and (g)"),"... ")
    malPrivKeyA, malPrivKeyB = findSecretKey (pubA,pubB,pubPrimeKey,pubBaseKey)
    print("    done")
    print("  calculates the secret key",pBlue("(k)"),"of ",pGreen("Alice and Bob"),"using the derived shared secrets",pBlue("(a,b)"),"and publicly known",pBlue("(A), (B), (p)"),"... ")
    thr_malSecretKeyA = ThreadWithReturnValue(target=calcKey, args=(malPrivKeyA, pubB, pubPrimeKey,))
    thr_malSecretKeyA.start()
    thr_malSecretKeyB = ThreadWithReturnValue(target=calcKey, args=(malPrivKeyB, pubA, pubPrimeKey,))
    ...
    checkAnswer("Alice & Bob shared secret using Alice's private key", malSecretKeyA, kA)
    checkAnswer("Alice & Bob shared secret using Bob's private key", malSecretKeyB, kB)
    print("NOTE: Their might be collisions (duplicate) private keys",pBlue("(a orb )"),"but knowing the shared secret would already be a massive gain")
