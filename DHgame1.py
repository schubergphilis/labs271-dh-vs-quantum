...
def main():
    ...
    try:
       gen_kA = _thread.start_new_thread(calcKey, ( privA, pubB, pubPrimeKey, ) )    
       gen_kB = _thread.start_new_thread(calcKey, ( privB, pubA, pubPrimeKey, ) )    
       gen_kA.join()
       kA = gen_kA.value
       gen_kB.join()       
       kB = gen_kB.value
    ...
    try:
       gen_calcSecretKeyA = _thread.start_new_thread(calcKey, ( calcPrivKeyA, pubB, pubPrimeKey, ) )    
       gen_calcSecretKeyB = _thread.start_new_thread(calcKey, ( calcPrivKeyB, pubA, pubPrimeKey, ) )  
       gen_calcSecretKeyA.join()
       calcSecretKeyA = gen_calcSecretKeyA.value
       gen_calcSecretKeyB.join()       
       calcSecretKeyB = gen_calcSecretKeyB.value  
    except:
       print("ERROR: unable to start thread")
       exit(1)     
    print ("  Done")
